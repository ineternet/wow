...
local Charsheet = Global.use"Object".inherit"Charsheet"

--Stat def for unit type (paper doll)

Charsheet.new = Global.Constructor(Charsheet, {
    level = 1,

    class = Const.Classes.Classless,
    race = Const.Races.None,
    spec = Const.Specs.None,

    equipment = Global.use"Equipment".new,
}, function(self)
...
Charsheet.canDualWield = function(self)
    return self.class == Const.Classes.Warrior or self.class == Const.Classes.Rogue
end

Charsheet.baseMana = function(self)
    local value = Const.BaseMana[self.level]
    if self.class == Const.Classes.Mage or self.class == Const.Classes.Priest or self.class == Const.Classes.Warlock then
...
Charsheet.isTank = function(self, unit)
    if self.spec == Const.Specs.Guardian then
        local isBear = unit:hasAura(Auras.BearForm)
        return isBear
    end
    return ({
        [Const.Specs.Protection] = true,
        [Const.Specs.Crusader] = true,
...
Charsheet.isStrengthSpec = function(self)
    return ({
        [Const.Specs.Protection] = true,
        [Const.Specs.Fury] = true,
        [Const.Specs.Crusader] = true,
        [Const.Specs.Guardian] = true,
    })[self.spec or 0] or false
end

Charsheet.isAgilitySpec = function(self)
    return ({
        [Const.Specs.Marksman] = true,
        [Const.Specs.Survival] = true,
        [Const.Specs.Assassin] = true,
        [Const.Specs.Combat] = true,
        [Const.Specs.Feral] = true,
    })[self.spec or 0] or false
end
...
for _, primStat in ipairs({"strength", "stamina", "agility", "intellect"}) do
    Charsheet[primStat] = function(self, unit)
        local classBase = math.round(Const.BasePrimaryStat(primStat, self.level, self.class, self.race))
...
Charsheet.attackPower = function(self, unit)
    ...
    if self.spec == Const.Specs.Restoration or self.spec == Const.Specs.Divination then
...
Charsheet.spellPower = function(self, unit)
    ...
    if self.spec == Const.Specs.Crusader then
        value = self:abilityDamage(true) * 1.01
    end
    if self.spec == Const.Specs.Feral or self.spec == Const.Specs.Guardian then
...
Charsheet.abilityDamage = function(self, unit, isMainHand)
    local value
    if isMainHand then
        value = math.round(self:totalMainHandDamage() / self.equipment.slots[Const.Slots.MainHand]:flat("weaponSpeed"))
    else
        value = math.round(self:totalOffHandDamage() / self.equipment.slots[Const.Slots.OffHand]:flat("weaponSpeed"))
    end
    value = value * 6
    if self:isTank() then
        value = value * (1 + self:mastery(unit))
...
Charsheet.diminishSecondaryStat = function(self, stat)
    ...
    local pctStat = (rating / Const.SecondaryRatingConversion[stat][self.level]) * 0.01
...
for _, tertStat in ipairs({"avoidance", "leech", "speed"}) do
    ...
    Charsheet[tertStat] = function(self, unit)
        ...
        local value = aAuraMod + math.min(mval, math.round(avd) / Const.SecondaryRatingConversion[tertStat][self.level] / 100)
        return value
    end
end
...
Charsheet.totalMainHandDamage = function(self)
    return self.equipment.slots[Const.Slots.MainHand]:flat("weaponDamage")
end

Charsheet.totalOffHandDamage = function(self)
    return self.equipment.slots[Const.Slots.OffHand]:flat("weaponDamage")
end

--Return GCD length in seconds, considering haste.
Charsheet.gcd = function(self, unit, gcdEnum)
    local haste = self:haste(unit)
    local gcd = Const.GCDTimeout[gcdEnum]
    return gcd / (1 + haste)
end
...
Charsheet.mitigate = function(self, unit, causerSheet, damage, school, isMassive)
    if school == Const.Schools.Physical then
